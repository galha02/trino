...
The `json_path` value is case-sensitive, while the SQL identifiers are
uppercase. Use quoted identifiers in the `PASSING` clause:
...
`FOR ORDINALITY` adds incremental row numbers to the table, starting from `1`.
